...
$(function() {
	...
	$('.onesteporder-products-total-submit, .onesteporder-products-total-submit-fast, .onesteporder-noframe-ordering-button').hover(function () {$(this).addClass("onesteporder-products-total-submit-hover");	}, 	function () {$(this).removeClass("onesteporder-products-total-submit-hover");});	
	...
	$('.onesteporder-contact-button').hover(function () {$(this).addClass("onesteporder-contact-button-hover");	}, 	function () {$(this).removeClass("onesteporder-contact-button-hover");});	
	...
	$('.onesteporder-remove-all-elements a, a.onesteporder-products-remove, a.onesteporder-contact-adresses').hover(function () {$(this).find('span').addClass("hover");	}, 	function () {$(this).find('span').removeClass("hover");});	
	...
	$('.onesteporder-checkbox').each(function(){
		...
		cb.next().prepend(function(){
			return (cb.is(':checked'))?'<span class="custom_checkbox custom_checkbox_active" />':'<span class="custom_checkbox" />';
		});
		cb.click(function(){
	...
	$('#first_nameStandart, #last_nameStandart, #ContactInformationStandart input, #ContactInformationStandart textarea').keyup(function()  { CheckAdressChanged('Standart'); });
...
function RadioStyle(type){	
	...
	$('input:radio[name='+type+'MethodID]').each(function(){
		...
		cb.addClass('radio');
		cb.next().prepend(function(){
			return (cb.is(':checked'))?'<span class="custom_radio custom_radio_active" />':'<span class="custom_radio" />';
		});
		cb.click(function(){
...
function SelectOrderingType(Url, Type){
	...
	$('#onesteporder-order-types-information-array'+Type).addClass('onesteporder-order-types-information-array-visible');
	if(Type == '1'){
		$('#FastOrdering').show();
...
function BillingMethods(){
	...
	var SID = ($("input:radio[name=shippingMethodID]:checked").val());
...
function ShippingMethods(countryID, zoneID, shippingAddressID){
	...
	if(countryID == undefined || isNaN(countryID) ){ countryID = '' };
	if(zoneID == undefined || isNaN(zoneID)){ zoneID = '' };
	if(shippingAddressID == undefined || isNaN(shippingAddressID) ){ shippingAddressID = '' };

	$.ajax({  
		type: "POST",
		data: "GetShippingMethods=yep&countryID="+countryID+"&zoneID="+zoneID+"&shippingAddressID="+shippingAddressID, 
		dataType : "json", 		
		success: function(ReturnHTML){ 
		...
		error: function(){StyleAction('ShippingMethods', 'end', false);	}
	});	
};
...
function UpdateShippingPrices(ReturnData){
	$.each(ReturnData.ShippingCosts, function(key, ShippingMethod) {
		if(ShippingMethod.length>1){
			...
			toHTML+= '<select name="shippingServiceID['+key+']"  id="shippingServiceID'+key+'" onchange="UpdateTotalPrice();" >';
			$.each(ShippingMethod, function(key, Cost) {
				toHTML+= '<option value="'+Cost.id+'"  xRate="'+Cost.rateWithOutUnit+'">'+Cost.name+' - '+Cost.rateWithUnit+'</option>';
			});
			toHTML+= '</select>';
			...
			if(ShippingMethod[0].rate>0){
				...
				if(ShippingMethod[0].rate !=''){
					toHTML+= '<span id="shippingCost'+key+'" xRate="'+ShippingMethod[0].rateWithOutUnit+'">'+ShippingMethod[0].rateWithUnit+'</span>';
				}
...
function UpdateTotalPrice(){
	...
	$('#TotalOrderPrice').html(formatPrice(parseFloat($('#TotalOrderPriceInput').val())+parseFloat(CurrentShippingCost)));
}
	...
	$.ajax({  
		...
		success: function(ReturnHTML){ 
			...
			if(ReturnHTML.isEmpty==true){
				$('#ShippingZoneTD'+type).html('<input name="shipping_address[state]" value="" type="text" id="stateStr'+type+'" onkeyup="CheckAdressChanged(\''+type+'\');" class="onesteporder-contact-input" />');
			}else{
				$('#ShippingZoneTD'+type).html('<select name="shipping_address[zoneID]" id="ShippingZone'+type+'" onChange="ChangeState(this, \''+type+'\');" class="onesteporder-contact-select" ></select>');
				$.each(ReturnHTML.zones, function(key, value) { 
					$('#ShippingZone'+type).append('<option value="'+value.zoneID+'">'+value.zone_name+'</option>');	
				});
			}
			if(type=="Standart") {
				$('#ShippingMethods').html(ReturnHTML.ReturnHTMLShipping);
		...
		error: function(){ StyleAction('ChangeCountry', 'end', ArrayToStyle);}
	});
	return false; 	
};
...
function ChangeBillingCountry(Select, type){
	...
	$.ajax({  
		...
		success: function(ReturnHTML){ 
			...
			if(ReturnHTML.isEmpty==true){
				$('#BillingZoneTD'+type).html('<input name="billing_address[state]" value="" type="text"  class="onesteporder-contact-input" />');
			}else{
				$('#BillingZoneTD'+type).html('<select name="billing_address[zoneID]" id="BillingZone'+type+'" class="onesteporder-contact-select" ></select>');
				$.each(ReturnHTML.zones, function(key, value) { 
					$('#BillingZone'+type).append('<option value="'+value.zoneID+'">'+value.zone_name+'</option>');	
				});
			}
			StyleAction('ChangeBillingCountry', 'end', ArrayToStyle);
		},
		error: function(){ StyleAction('ChangeBillingCountry', 'end', ArrayToStyle);}
	});
	return false; 	
};
	...
	$.ajax({  
		...
		success: function(ReturnData){ 
			...
			if(ReturnData.isEmpty==true){
				$('#ShippingZoneTD'+type).html('<input name="shipping_address[state]" value="'+state+'" type="text" id="stateStr'+type+'"  onkeyup="CheckAdressChanged(\''+type+'\');" class="onesteporder-contact-input" />');
			}else{
				$('#ShippingZoneTD'+type).html('<select name="shipping_address[zoneID]" id="ShippingZone'+type+'" onChange="ChangeState(this, \''+type+'\');" class="onesteporder-contact-select" ></select>');
				$.each(ReturnData.zones, function(key, value) { 
					if(value.zoneID == zoneID){ var checked = 'selected'; }
					$('#ShippingZone'+type).append('<option value="'+value.zoneID+'" '+checked+'>'+value.zone_name+'</option>');	
				});
			}
			if(type=="Standart") {
		...
		error: function(){ StyleAction('UpdateUserAdress', 'end', ArrayToStyle);}
	});
	return false; 
...
function RemoveElement(elementID){
	...
	$.ajax({  
		...
		error: function(){ StyleAction('RemoveElement', 'end', ArrayToStyle); }
	});
	return false; 	
};
		...
		success: function(ReturnData){ 		
			...
			if(ReturnData.ProductData){
				...
				if(ReturnData.ProductData.updateElement){	
					...
					var configuration_item_url = $('#CartElement'+oldItem+' .onesteporder-configuration-item-url a').attr("onclick").replace(Item, newItem);
					$('#CartElement'+oldItem+' .onesteporder-configuration-item-url a').attr("onclick", configuration_item_url); 	 	
					$('#CartElement'+oldItem+' .onesteporder-configuration-item-div').attr('id','onesteporder-configuration-item-div-'+newItem); 
					
					var configuration_item_submit = $('#CartElement'+oldItem+' .configuration-item-submit').attr("onclick").replace(Item, newItem);
					$('#CartElement'+oldItem+' .configuration-item-submit').attr("onclick", configuration_item_submit); 		
					
					var products_count_icon_p = "RecalculateCartIcons(true, '"+newItem+"','"+ReturnData.ProductData.costUC+"','"+in_stock+"');";
					var products_count_icon_m = "RecalculateCartIcons(false, '"+newItem+"','"+ReturnData.ProductData.costUC+"','"+in_stock+"');";
					
					$('#CartElement'+oldItem+' .onesteporder-products-count-icon-p').attr("onclick", products_count_icon_p); 
					$('#CartElement'+oldItem+' .onesteporder-products-count-icon-m').attr("onclick", products_count_icon_m); 
					$('#CartElement'+oldItem+' .onesteporder-products-count-icon-p').addClass('system_RecalculateCartIconP'+newItem).removeClass('system_RecalculateCartIconP'+oldItem);
					$('#CartElement'+oldItem+' .onesteporder-products-count-icon-m').addClass('system_RecalculateCartIconM'+newItem).removeClass('system_RecalculateCartIconM'+oldItem);
					
					var RemoveElement = "RemoveElement('"+newItem+"');";
					$('#RemoveElement'+oldItem).attr("onclick", RemoveElement).attr('id','RemoveElement'+newItem); 
					
					$('#CartElement'+oldItem+' .onesteporder-products-name a:first').html(ReturnData.ProductData.name);
					$('#ProductPrice'+oldItem).html(ReturnData.ProductData.cost).attr('id','ProductPrice'+newItem); 		
					
					$('#CartElement'+oldItem+' .onesteporder-products-count-input').val(ReturnData.ProductData.quantity); 
					var RecalculateCart = "RecalculateCart(this,'"+newItem+"','"+ReturnData.ProductData.costUC+"','"+in_stock+"');";
					$('#CartElement'+oldItem+' .onesteporder-products-count-input').attr("onBlur", RecalculateCart).attr('name','count_'+newItem).attr('id','ProductQty'+newItem); 
...
function RecalculateCart(input, elementID, costUC, in_stock){
	...
	$.ajax({  
		type: "POST",
		data: "RecalculateCart=yep&element="+elementID+"&count="+$(input).val()+"&costUC="+parseFloat(costUC)+"&countryID="+countryID+"&zoneID="+zoneID,  
		dataType : "json", 
		success: function(ReturnData){ 			
...
function DeleteCoupon(){
	...
	$.ajax({  
		...
		error: function(){ StyleAction('DeleteCoupon', 'end', ''); }
	});
	return true; 	
};		
...
function CheckBeforeSubmit(form){
	...
	$.ajax({  
		...
		error: function(){StyleAction('CheckBeforeSubmit', 'end', '');}
	}); 
	return false;
};		
	...
	switch (Action) {
		...
		case 'UpdateUserAdress':
			if(Type=='start'){
				...
				$("#ShippingCountry"+Array['type']+",#ShippingZone"+Array['type']+",#stateStr"+Array['type']+",.system_ChangeAdressButtons").attr('disabled','true');
				$(".system_ChangeAdressButtons").addClass('onesteporder-contact-button-disabled');
				$('#ShippingFirstNameTR'+Array['type']+',#ShippingLastNameTR'+Array['type']+',#ShippingZipTR'+Array['type']+',#ShippingCityTR'+Array['type']+',#ShippingAdressTR'+Array['type']+',#ShippingZoneTR'+Array['type']+',#ShippingCountryTR'+Array['type']).fadeTo('fast',.5); 
				$('#ShippingFirstNameTR'+Array['type']+' input,#ShippingLastNameTR'+Array['type']+' input,#ShippingZipTR'+Array['type']+' input, #ShippingCityTR'+Array['type']+' input,#ShippingAdressTR'+Array['type']+' textarea').attr('disabled','true');
			}else{		
				$('#ShippingFirstNameTR'+Array['type']+',#ShippingLastNameTR'+Array['type']+',#ShippingZipTR'+Array['type']+',#ShippingCityTR'+Array['type']+',#ShippingAdressTR'+Array['type']+',#ShippingZoneTR'+Array['type']+',#ShippingCountryTR'+Array['type']).fadeTo('fast',1); 
				$('#ShippingFirstNameTR'+Array['type']+' input,#ShippingLastNameTR'+Array['type']+' input,#ShippingZipTR'+Array['type']+' input, #ShippingCityTR'+Array['type']+' input,#ShippingAdressTR'+Array['type']+' textarea').removeAttr('disabled');
				$("#ShippingCountry"+Array['type']+",#ShippingZone"+Array['type']+",#stateStr"+Array['type']+",.system_ChangeAdressButtons").removeAttr('disabled');	
				$(".system_ChangeAdressButtons").removeClass('onesteporder-contact-button-disabled');
		...
		case 'RecalculateCart':
			if(Type=='start'){
				...
				$('.system_RecalculateCartIconP'+Array['elementID']+', .system_RecalculateCartIconM'+Array['elementID']+'').attr('disabled','true');
				$('.system_RecalculateCartIconP'+Array['elementID']).addClass('onesteporder-products-count-icon-p-disabled');
				$('.system_RecalculateCartIconM'+Array['elementID']).addClass('onesteporder-products-count-icon-m-disabled');
				...
				if($(Array['input']).val()==1){
					...
					$('.system_RecalculateCartIconP'+Array['elementID']).removeClass('onesteporder-products-count-icon-p-disabled');
				}else if($(Array['input']).val()==Array['in_stock']){
					$('.system_RecalculateCartIconM'+Array['elementID']).removeAttr('disabled');
					$('.system_RecalculateCartIconM'+Array['elementID']).removeClass('onesteporder-products-count-icon-m-disabled');
				}else{				
					$('.system_RecalculateCartIconP'+Array['elementID']+', .system_RecalculateCartIconM'+Array['elementID']).removeAttr('disabled');
					$('.system_RecalculateCartIconP'+Array['elementID']).removeClass('onesteporder-products-count-icon-p-disabled');
					$('.system_RecalculateCartIconM'+Array['elementID']).removeClass('onesteporder-products-count-icon-m-disabled');
				}
